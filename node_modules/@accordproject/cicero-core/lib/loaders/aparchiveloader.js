...
class ApArchiveLoader extends HTTPArchiveLoader {
  ...
  load(url, options) {
    ...
    if (atIndex < 0 || hashIndex < 0) {
      throw new Error("Invalid template specifier. Must contain @ and #: ".concat(url));
    }

    const templateName = url.substring(5, atIndex);
    const templateVersion = url.substring(atIndex + 1, hashIndex);
    const rewrittenUrl = "https://templates.accordproject.org/archives/".concat(templateName, "@").concat(templateVersion, ".cta");
